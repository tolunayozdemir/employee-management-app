...
suite('Employee Reducer', () => {
  ...
  suite('DELETE_EMPLOYEE', () => {
    test('should handle deleting an employee', () => {
      ...
      expect(employeeReducer(currentState, action)).to.deep.equal(expectedState);
    });
  ...
  suite('UPDATE_EMPLOYEE', () => {
    ...
    test('should not modify state if employee to update is not found', () => {
      ...
      expect(employeeReducer(currentState, action).employees).to.have.lengthOf(1);
      expect(employeeReducer(currentState, action).employees[0]).to.deep.equal(currentState.employees[0]);
    });
  });
  ...
  suite('SET_LANGUAGE', () => {
    test('should handle setting the language', () => {
      ...
      expect(employeeReducer(initialState, action)).to.deep.equal(expectedState);
    });
  });
});